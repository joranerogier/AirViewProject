...
vme2df <- function(obs_data, U) {
  ...
  for (l in 1:length(unique(obs_data$id_cell))) {
    ...
    pol_conc <- obs_data[which(obs_data$id_cell == cell), 2]$pol
    #N <- length(pol_conc)
    N <- as.integer(obs_data[which(obs_data$id_cell == cell), 5]$Point_Count)
    # VME calculated for grid cells with more than one obs 
    if (N > 1) {
      var1 = (obs_data[which(obs_data$id_cell == cell), 1]$std_no2_data / sqrt(N))**2 
      var2 = (U ** 2 / N) * obs_data[which(obs_data$id_cell == cell), 4]$sum_no2_data_squared
...
file_airview <- "AMS_fishnet_NSL_100m_SumarizeWithin_sd_squaredsum_NO2Jul_ConcLTE50_points_.csv"#
...
(names(airview_ams)[names(airview_ams) == "mean_no2_data"] <- "pol")
...
to.remove <- c( "IOD", "sum_log_conc","mean_log_conc","ORIG_FID", "sum_no2_log_squared", "OID_")
...
vario_res_drift=vario.calc(data2.db, lag=50, nlag=25) # experimental variogram on residuals
...
unique.neigh=neigh.create(ndim=2,type=2, nmini=15, nmaxi = 100, radius=c(30000,30000))# TODO: check this -> orginally this:neigh.create(type=0,ndim=2)
