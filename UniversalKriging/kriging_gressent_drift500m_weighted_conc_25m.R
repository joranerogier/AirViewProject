...
vme2df <- function(obs_data, U) {
  ...
  for (l in 1:length(unique(obs_data$id_cell))) {
    ...
    if (point_count > 1) {
      var1 = (obs_data[which(obs_data$id_cell == cell), 2]$std_no2_data / sqrt(N))**2 
...
vario_res_drift=vario.calc(data2.db, lag=25, nlag=20) # experimental variogram on residuals
...
unique.neigh=neigh.create(ndim=2,type=2, nmini=2, nmaxi = 25, radius=c(300,300))# TODO: check this -> orginally this:neigh.create(type=0,ndim=2)
