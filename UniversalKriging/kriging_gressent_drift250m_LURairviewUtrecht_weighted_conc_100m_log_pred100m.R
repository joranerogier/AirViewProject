...
vme2df <- function(obs_data, U) {
  ...
  for (l in 1:length(unique(obs_data$id_cell))) {
    ...
    N <- point_count + as.integer(obs_data[which(obs_data$id_cell == cell), 5]$sum_drivedays)
    ...
    if (point_count > 1) {
      var1 = (obs_data[which(obs_data$id_cell == cell), 3]$std_no2_data / sqrt(N))**2 
...
# Situation, will be used to give name to files
drift_data = 'NSL'
drift_cell_size = 250
drift_origin_city = 'AMS'
obs_data = 'AV'
obs_type = '100m_avg_points'
file_tag = paste(drift_data, '_', drift_cell_size, 'm_' , drift_origin_city, '_', obs_data, '_', obs_type, sep='')

########################################################
#       For drift, lower resolution to reduce variance #
########################################################
...
file_drift <- "fishnet_AMS_250_NSL_LUR_predictions/fishnet_AMS_250m_NSL_LUR_predictions.shp"                          # Drift file
file_drift_points <- "fishnet_AMS_250m_NSL_LUR_predictions_points.csv"
file_airview <- "AMS_fishnet_100m_summarize_weighted_conc_points.csv"
...
(names(drift)[names(drift) == "mean_mean_"] <- "pol")
drift_sp <- as(drift, Class="Spatial")
(names(drift_sp)[names(drift_sp) == "mean_mean_"] <- "pol")


###############################################
#      For higher resolution prediction       #
###############################################
indir_drift_points_highres_prediction <- "C:/Users/Jorane Rogier/Documents/Master/DSthesis/Data/GridData/" 
file_drift_points_highres_prediction <- "fishnet_airview_AMS_100m_LUR_predictions_points.csv"
indir_drift_highres_prediction <- "C:/Users/Jorane Rogier/Documents/Master/DSthesis/Data/GridData/fishnet_airview_AMS_100m_LUR_predictions/"
file_drift_highres_prediction <- "fishnet_airview_AMS_100m_LUR_predictions.shp"               

# Read drift file (which is a .shp file)
# This is done here, to get the parameters from the grid definition
drift_highres_prediction <- st_read(paste0(indir_drift_highres_prediction,file_drift_highres_prediction))
(names(drift_highres_prediction)[names(drift_highres_prediction) == "mean_nsl_l"] <- "pol")
drift_sp_highres_prediction <- as(drift_highres_prediction, Class="Spatial")
(names(drift_sp_highres_prediction)[names(drift_sp_highres_prediction) == "mean_nsl_l"] <- "pol")
...
ncols_interpolation = (xmax-xmin)/res_interpolation
nrows_interpolation = (ymax-ymin)/res_interpolation
...
# This grid has a higher resolution than the drift grid!
print("CREATE GRID")
...
grid_drift=expand.grid(X,Y) # create grid from x and y columns
data2 <- rep(0,length(grid_drift)) # first attribute pseudo-data
grid_drift$data2 <- data2
grid_drift = subset(grid_drift, Var1 >= xmin & Var1 <= xmax & Var2 >=ymin & Var2 <= ymax) # subset full grid to the domain geographical limits if necessary
coordinates(grid_drift)=~Var1+Var2 # convert grid to a spatial object
proj4string(grid_drift)=CRS_RDnew # define grid projection
grid_coords_drift = grid_drift@coords # extract coordinates


# Define a temporary grid (grid_tmp) as the grid to use for the interpolation
grid_tmp_drift <- data.frame(lon=seq(1:length(grid_coords_drift[,1])),
                        lat=seq(1:length(grid_coords_drift[,1])),
                        Drift=seq(1:length(grid_coords_drift[,1])))
grid_tmp_drift$lon <- grid_coords_drift[,1]
grid_tmp_drift$lat <- grid_coords_drift[,2]
grid_tmp_drift$Drift = grid_tmp_drift$Drift*1 # add pseudo drift to the grid (no matter the values)
grid_tmp_drift$Long <- grid_tmp_drift$lon; grid_tmp_drift$Lat <- grid_tmp_drift$lat
coordinates(grid_tmp_drift)=~Long+Lat
proj4string(grid_tmp_drift)=CRS_RDnew # define projection

#####################################
#        GRID FOR HIGH RES PRED     #         
#####################################
...
X<-c() # x vector
for (i in 1:ncols_interpolation){if (i==1){X[i]=xmin}else{X[i]=X[i-1]+res_interpolation}}
Y<-c() # y vector
for (i in 1:nrows_interpolation){if (i==1){Y[i]=ymin}else{Y[i]=Y[i-1]+res_interpolation}}
grid_high_res = expand.grid(X,Y) # create grid from x and y columns
data1 <- rep(0,length(grid_high_res)) # first attribute pseudo-data
grid_high_res$data1 <- data1
grid_high_res = subset(grid_high_res, Var1 >= xmin & Var1 <= xmax & Var2 >=ymin & Var2 <= ymax) # subset full grid to the domain geographical limits if necessary
coordinates(grid_high_res)=~Var1+Var2 # convert grid to a spatial object
proj4string(grid_high_res)=CRS_RDnew # define grid projection
grid_coords_high_res = grid_high_res@coords # extract coordinates


# Define a temporary grid (grid_tmp_high_res) as the grid to use for the interpolation
grid_tmp_high_res <- data.frame(lon=seq(1:length(grid_coords_high_res[,1])),
                       lat=seq(1:length(grid_coords_high_res[,1])),
                       Drift=seq(1:length(grid_coords_high_res[,1])))
grid_tmp_high_res$lon <- grid_coords_high_res[,1]
grid_tmp_high_res$lat <- grid_coords_high_res[,2]
grid_tmp_high_res$Drift = grid_tmp_high_res$Drift*1 # add pseudo drift to the grid (no matter the values)
grid_tmp_high_res$Long <- grid_tmp_high_res$lon; grid_tmp_high_res$Lat <- grid_tmp_high_res$lat
coordinates(grid_tmp_high_res)=~Long+Lat
proj4string(grid_tmp_high_res)=CRS_RDnew # define projection
...
(names(drift_points)[names(drift_points) == "mean_mean_nsl_lur_no2_predictions"] <- "pol")
drift_points_df <- as.data.frame(drift_points)
drift_points_df <- subset(drift_points_df, select=-c(sum_Area_SQUAREKILOMETERS, Polygon_Count, OID_,ORIG_FID))
...
# create raster 
ras_drift=raster(list(x=sort(unique(coordinates(grid_tmp_drift)[,1])),y=sort(unique(coordinates(grid_tmp_drift)[,2])),z=matrix(drift$pol,nrow=length(sort(unique(coordinates(grid_tmp_drift)[,1]))),byrow=F))) # create a raster
plot(ras_drift)
...
airview_ams <- airview_ams[airview_ams$pol <= 50,]
airview_ams$pol <- log(airview_ams$pol)
...
#data_sub <- airview_ams[airview_ams$pol <= 50,]
...
zl_drift <- c(min(drift_points$pol)-1,max(drift_points$pol)+1)
...
grid.db=db.create(grid2[,c("lon","lat","Drift")],flag.grid=F,autoname=F)# grid2 is the grid for higher resolution
...
# Color and scale residual data
data_plt <- data_RDnew
zl_res <- c(min(data_plt$resvcdrift),max(data_plt$resvcdrift))
data_plt@data$resvcdrift[which(data_plt@data$resvcdrift < zl_res[1])] <- zl_res[1]+0.1
data_plt@data$resvcdrift[which(data_plt@data$resvcdrift > zl_res[2])] <- zl_res[2]-0.1
ncol3=20
mycol_ncol3 <- colorRampPalette(mycol)(ncol3)
brks3 <- seq(zl_res[1], zl_res[2], length.out = ncol3+1)
cut_res <- cut(data_plt@data$resvcdrift,brks3)
col_res <- mycol_ncol3[cut_res]

par(mar=c(7,9,2,9)) # margin bot left top right (need space for the lgd
# ras_drift plotting necessary to start plot and be able to display residuals
plot(ras_drift, col='white', interpolate=FALSE, maxpixels=500000000,
     main="Regression residuals", cex.main=2,
     xlab="Longitude",ylab="Latitude", cex.lab=1.5, cex.axis=1.5)
plot(data_plt,pch=21,col='black',bg=col_res,add=T,cex=0.75)
image.plot(legend.only=TRUE, add=TRUE, horizontal = FALSE, zlim=zl_res,breaks=brks3,
           col=mycol_ncol3, legend.shrink=.7,legend.width=1.,legend.mar=12,
           legend.args=list(text='Residual',cex=1.5,line=1,font=1,col='black'),
           axis.args=list(at=brks3,labels=format(brks3,digits=2),cex.axis=1,col.axis="black",col="black"))

dev.off()

hist(data_plt$resvcdrift, breaks=30, main="Histogram of regression residuals", xlab='residuals')
...
# Color and scale vmap data
data_plt <- vm
zl_vmap <- c(min(vm@items$VMAP.resvcdrift),max(vm@items$VMAP.resvcdrift))
data_plt@items$VMAP.resvcdrift[which(data_plt@items$VMAP.resvcdrift < zl_vmap[1])] <- zl_vmap[1]+0.1
data_plt@items$VMAP.resvcdrift[which(data_plt@items$VMAP.resvcdrift > zl_vmap[2])] <- zl_vmap[2]-0.1
ncol4=20
mycol_ncol4 <- colorRampPalette(mycol)(ncol4)
brks4 <- seq(zl_vmap[1], zl_vmap[2], length.out = ncol4+1)
cut_vmap <- cut(data_plt@items$VMAP.resvcdrift,brks4)
col_vmap <- mycol_ncol4[cut_vmap]

par(mar=c(7,9,2,9)) # margin bot left top right (need space for the lgd
plot(vm,npairdw=TRUE,npairpt=TRUE,cex.main=1.2,cex.lab=1.2, cex.axis=1.2, mar=c(2,2,2,20))
image.plot(legend.only=TRUE, add=TRUE, horizontal = FALSE, zlim=zl_vmap,breaks=brks4,
           col=mycol_ncol4, legend.shrink=.7,legend.width=1.,legend.mar=12,
           legend.args=list(text='Variance of residuals',cex=1.5,line=1,font=1,col='black'),
           axis.args=list(at=brks4,labels=format(brks4,digits=2),cex.axis=1,col.axis="black",col="black"))
dev.off()

# 4) Calculate the variogram based on the residuals
vario_res_drift=vario.calc(data2.db, lag=100, nlag=12) # experimental variogram on residuals
...
par(mar=c(7,9,2,9)) # margin bot left top right (need space for the lgd
plot(vario_res_drift, npairdw=TRUE,  npairpt=TRUE,main = "b)",ylim=c(0,0.15),lwd=3,cex.main=2,xlab="Distance h (m)",ylab="γ(h)",cex.lab=1.8, cex.axis=1.8)
...
write.table(grid,file=paste(outdir2,"EDK_grid_drift250_AirViewUtrecht_min3dd_lag100_nlags40_uniqueneigh_comma",'.csv',sep=''),row.names=F,col.names=T,sep=',')
#grid <- read_csv(file=paste0(outdir2,"EDK_grid_",city,"_",pol,"_",estim_YYYY,"_",kriging_data,'.csv'))
#View(krige_ED)

typeof(grid$Pred_EDK)
...
t <- read_csv(file=paste(outdir2,"EDK_grid_drift250_AirViewUtrecht_min3dd_lag100_nlags40_uniqueneigh_comma",'.csv',sep=''))
grid$Pred_EDK
