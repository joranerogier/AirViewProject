...
# Directory paths
indir_drift <- "C:/Users/Jorane Rogier/Documents/Master/DSthesis/Data/GridData/fishnet_AMS_NSL_100m_LUR_predictions/"
indir_drift_points <- "C:/Users/Jorane Rogier/Documents/Master/DSthesis/Data/GridData/" 
indir_airview <- "C:/Users/Jorane Rogier/Documents/Master/DSthesis/Data/GoogleAir/Amsterdam/"#intersection/
file_drift <- "fishnet_AMS_NSL_100m_SummarizeWithin_NSL_LUR_predictions.shp"                                                            # Drift file
file_drift_points <- "fishnet_AMS_NSL_100m_NSL_LUR_predictions_points.csv"
file_airview <- "AMS_DataMixed_NO2_5Jul_MidPoints_ConcLTE50_gte3dd.csv"#
...
airview_ams <- airview_ams[sample(nrow(airview_ams),10000),]

## Not anymore: now subset of original data is taken, where a point has 10+ driving days
colnames(airview_ams)
to.remove <- c("OID_", "NO2_Data", "NO2_Mixed", "DriveDays", "ROAD_FID","NO2_LUR","NO2_SE","NO2_Diff","NO2_SEp","Shape_Leng","ORIG_FID", "weighted_conc", "NO2_squared"  )
...
vario_res_drift=vario.calc(data2.db)#, lag=1000, nlag=15) # experimental variogram on residuals
...
unique.neigh=neigh.create(ndim=2,type=2, nmini=2, nmaxi = 200, radius=c(10000,10000))# TODO: check this -> orginally this:neigh.create(type=0,ndim=2)
...
#write.table(grid,file=paste(outdir2,"EDK_grid_corrected_grid_100m_",city,"_",pol,"_",estim_YYYY,"_",kriging_data,'.csv',sep=''),row.names=F,col.names=T,sep=';')
...
print(ras_Pred)
View(ras_Pred)
ras_Pred@data@values

prediction_df <- data.frame("pred"=ras_Pred@data@values, "x"=grid.db@items$lon, "ÿ"=grid.db@items$lat)

write.csv(prediction_df, paste0(outdir2, "predictions_gressent_drift_100m_airview_mte3dd_lte60_autoVarioCalc_minn2maxn200radius10000.csv"))
...
zl_error <- c(min(StDev_EDK2),max(StDev_EDK2))
...
save(data2,file=paste0(outdir2,"EDK_data_",city,"_",pol,"_",estim_YYYY,"_",kriging_data,".Rda"))
