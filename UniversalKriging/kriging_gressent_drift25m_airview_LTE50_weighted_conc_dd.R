...
vme2df <- function(obs_data, U) {
  ...
  for (l in 1:length(unique(obs_data$id_cell))) {
    ...
    pol_conc <- obs_data[which(obs_data$id_cell == cell), 8]$pol
    #N <- length(pol_conc)
    point_count <- as.integer(obs_data[which(obs_data$id_cell == cell), 7]$Point_Count)
    N <- point_count + as.integer(obs_data[which(obs_data$id_cell == cell), 6]$sum_drivedays)
    ...
    if (point_count > 1) {
      ...
      var2 = (U ** 2 / N) * obs_data[which(obs_data$id_cell == cell), 5]$sum_no2_squared
...
model="NSL-LUR-drift100m-airview-1or2_dd"                    # Dispersion model
...
file_airview <- "fishnet_AMS_25m_SummarizeWithin_AMS_DataMixed_NO2_5Jul_MidPoints_3_5_drivingdays_concLTE60_points.csv"#
...
length(data_sub$pol)
data_sub <- airview_ams[airview_ams$pol < 60,]
length(data_sub$pol)
...
rm(drift_sp)#bit of memory savings
...
vario_res_drift=vario.calc(data2.db, lag=500) # experimental variogram on residuals
...
unique.neigh=neigh.create(ndim=2,type=2, nmini=2, nmaxi = 50, radius=c(2000,2000))# TODO: check this -> orginally this:neigh.create(type=0,ndim=2)
...
write.table(grid,file=paste(outdir2,"EDK_grid_grid_25m_airview_weighted_25m_",city,"_",pol,"_",estim_YYYY,"_",kriging_data,'.csv',sep=''),row.names=F,col.names=T,sep=';')
...
save(data2,file=paste0(outdir2,"EDK_data_gt7res_",city,"_",pol,"_",estim_YYYY,"_",kriging_data,".Rda"))
