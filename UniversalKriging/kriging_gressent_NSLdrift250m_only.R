...
vme2df <- function(obs_data, U) {
  ...
  for (l in 1:length(unique(obs_data$id_cell))) {
    ...
    pol_conc <- obs_data[which(obs_data$id_cell == cell), 5]$pol
    N <- length(pol_conc)
...
indir_nsl <- "C:/Users/Jorane Rogier/Documents/Master/DSthesis/Data/NSL/"#
...
nsl_ams <-read_csv2(file=paste0(indir_nsl, file_nsl))
(names(nsl_ams)[names(nsl_ams) == "CONC_NO2"] <- "pol")

data_sub <- nsl_ams[nsl_ams$pol != 0,]

data_sf <- data_sub %>%
  st_as_sf(coords = c('X','Y'), crs = 28992, remove=FALSE) %>%
...
# aggregate
data_ms <- aggregate(.~X+Y,data=data_join,mean,na.action=na.pass) # aggregate on location
...
data_ms <- aggregate(.~X+Y,data=data_ms,mean,na.action=na.pass) # mean over position to avoid duplicate data on a similar location ==> error for kriging

# Convert to spatial object 
data_ms$Long <- data_ms$X
data_ms$Lat <- data_ms$Y
coordinates(data_ms) <- ~Long + Lat
proj4string(data_ms) <- CRS_RDnew
data_ms
data_ms <- subset(data_ms, select = -c(X, Y, vme_pol_msens_all, id_cell))
...
data.db=db.create(data_RDnew[,c("lon","lat","pol","vme")],flag.grid=F,ndim=2,autoname=F)
data.db=db.locate(data.db,'vme',"v",1)
grid.db=db.create(grid[,c("lon","lat")],flag.grid=F,autoname=F)# grid is also in RDnew
...
data2.db=db.create(data_RDnew[,c("lon","lat","pol","vme")],flag.grid=F,ndim=2,autoname=F) # create database
...
vario_res_drift=vario.calc(data2.db, lag=500, nlag=40) # experimental variogram on residuals
...
par(mar=c(7,9,2,9)) # margin bot left top right (need space for the lgd
plot(vario_res_drift, npairdw=TRUE,  npairpt=TRUE,main = "b)",ylim=c(0,50),lwd=3,cex.main=2,xlab="Distance h (m)",ylab="γ(h)",cex.lab=1.8, cex.axis=1.8)
...
krige_ED=kriging(data2.db, grid.db,model=vario_res_drift_model,uc=c("1"),neigh=unique.neigh, calcul = "point") # do kriging
...
write.table(grid,file=paste(outdir2,"EDK_grid_nodrift_250m_nsl_points_lag500_n40_uniqueneigh_comma",'.csv',sep=''),row.names=F,col.names=T,sep=',')
