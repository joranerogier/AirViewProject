...
## remove "outliers" based on z-score -> remove every sample with z > 3
#boxplot(airview_ams$pol)
airview_df <- as.data.frame(airview_ams$pol)
(names(airview_df)[names(airview_df) == "airview_ams$pol"] <- "pol")
z_scores <- as.data.frame(sapply(airview_df, function(airview_df) (abs(airview_df-mean(airview_df))/sd(airview_df))))    
idx <- which(z_scores > 3)
airview_ams <- airview_ams[-idx,]
#no_outliers <- z_scores[!rowSums(z_scores>3), ]
#head(no_outliers)

#length(no_outliers)
#length(airview_ams$pol)
#head(airview_df)
#Q1 <- quantile(airview_df$pol, probs=.25)
#Q3 <- quantile(airview_df$pol, .75)
#IQR <- IQR(airview_df$pol)
#no_outliers_iqr <- subset(airview_df, airview_df$pol > (Q1 - 1.5* IQR) & airview_df$pol < (Q3 + 1.5*IQR))
#dim(no_outliers_iqr)
...
png(filename=paste0(outdir,"Map_variogram_drift100_ariview100_",city,"_",pol,"_",estim_YYYY,"_",kriging_data,".png"), width=900, height=600, type="cairo",bg = "white") # do plot
...
vario_res_drift=vario.calc(data2.db, lag=100, nlag=20) # experimental variogram on residuals
...
write.table(grid,file=paste(outdir2,"EDK_grid_drift100mNSLLUR_lag100n20_uniqueneigh_zscoreOutliers",city,"_",pol,"_",estim_YYYY,"_",kriging_data,'.csv',sep=''),row.names=F,col.names=T,sep=';')
...
save(data2,file=paste0(outdir2,"EDK_EDK_grid_drift100mNSLLUR_lag100n40_uniqueneigh_",city,"_",pol,"_",estim_YYYY,"_",kriging_data,".Rda"))

# Do plot
png(filename=paste0(outdir,"CV_EDK_grid_drift100mNSLLUR_lag100n40_uniqueneigh_",city,"_",pol,"_",estim_YYYY,"_",kriging_data,"_VME.png"), width=1200, height=800, type="cairo",bg = "white")
