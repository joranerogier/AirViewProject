...
vme2df <- function(obs_data, U) {
  ...
  for (l in 1:length(unique(obs_data$id_cell))) {
    ...
    pol_conc <- obs_data[which(obs_data$id_cell == cell), 1]$pol
    N <- length(pol_conc)
...
file_airview <- "AMS_DataMixed_NO2_5Jul_MidPoints_10plusDrivingDays_1.csv"
...
file_drift_highres_prediction <- "fishnet_airview_AMS_100m_LUR_predictions.shp"                                                            # Drift file
...
(names(drift)[names(drift) == "mean_nsl_l"] <- "pol")
...
grid2=expand.grid(X,Y) # create grid from x and y columns
data2 <- rep(0,length(grid2)) # first attribute pseudo-data
grid2$data2 <- data2
grid2 = subset(grid2, Var1 >= xmin & Var1 <= xmax & Var2 >=ymin & Var2 <= ymax) # subset full grid to the domain geographical limits if necessary
coordinates(grid2)=~Var1+Var2 # convert grid to a spatial object
proj4string(grid2)=CRS_RDnew # define grid projection
grid_coords2 = grid2@coords # extract coordinates


# Define a temporary grid (grid_tmp) as the grid to use for the interpolation
grid_tmp2 <- data.frame(lon=seq(1:length(grid_coords2[,1])),
                       lat=seq(1:length(grid_coords2[,1])),
                       Drift=seq(1:length(grid_coords2[,1])))
grid_tmp2$lon <- grid_coords2[,1]
grid_tmp2$lat <- grid_coords2[,2]
grid_tmp2$Drift = grid_tmp2$Drift*1 # add pseudo drift to the grid (no matter the values)
grid_tmp2$Long <- grid_tmp2$lon; grid_tmp2$Lat <- grid_tmp2$lat
coordinates(grid_tmp2)=~Long+Lat
proj4string(grid_tmp2)=CRS_RDnew # define projection
...
(names(drift_points)[names(drift_points) == "mean_nsl_lur_no2_predictions"] <- "pol")
drift_points_df <- as.data.frame(drift_points)
drift_points_df <- subset(drift_points_df, select=-c(OID_,Point_Count,ORIG_FID))
...
# create raster 
ras_drift=raster(list(x=sort(unique(coordinates(grid_tmp2)[,1])),y=sort(unique(coordinates(grid_tmp2)[,2])),z=matrix(drift$pol,nrow=length(sort(unique(coordinates(grid_tmp2)[,1]))),byrow=F))) # create a raster
plot(ras_drift)
...
#####################################
#             READ DRIFT            # 
#       HIGH RES PREDICTION GRID    #
#####################################

print("READ DRIFT")

# Also read the drift points file
drift_points_pred_grid <- read_csv2(file=paste0(indir_drift_points_highres_prediction, file_drift_points_highres_prediction))
(names(drift_points_pred_grid)[names(drift_points_pred_grid) == "mean_nsl_lur_no2_predictions"] <- "pol")
drift_points_df_pred_grid <- as.data.frame(drift_points_pred_grid)
drift_points_df_pred_grid <- subset(drift_points_df_pred_grid, select=-c(OID_,Point_Count,ORIG_FID))
drift_points_df_pred_grid$x <- as.numeric(drift_points_df_pred_grid$POINT_X)
drift_points_df_pred_grid$y <- as.numeric(drift_points_df_pred_grid$POINT_Y)
coordinates(drift_points_df_pred_grid)=~x+y
proj4string(drift_points_df_pred_grid)=CRS_RDnew 
drift_points_df_pred_grid <- spTransform(drift_points_df_pred_grid,CRS_RDnew)
drift_points_df_pred_grid_wgs84 <- spTransform(drift_points_df_pred_grid,CRS_WGS84)
...
(names(airview_ams)[names(airview_ams) == "NO2_Data"] <- "pol")
...
to.remove <- c("OID_","ROAD_FID", "NO2_LUR","NO2_Mixed","NO2_SE","NO2_Diff",  
               "NO2_SEp","DriveDays","Shape_Leng", "ORIG_FID")
...
### WATCH OUT GRID_TMP WRONGLY SET
grid <- grid_tmp2
grid2 <- grid_tmp
...
#grid2$Drift <- drift_sp$pol

# 

grid2$Drift <- drift_sp_highres_prediction$mean_mean_
...
vario_res_drift=vario.calc(data2.db, lag=25, nlag=15) # experimental variogram on residuals
...
unique.neigh=neigh.create(ndim=2,type=2, nmini=5, nmaxi = 50, radius=c(250,250))# TODO: check this -> orginally this:neigh.create(type=0,ndim=2)
...
grid2$Pred_EDK=db.extract(krige_ED,'Kriging.pol.estim') # estimation
grid2$StDev_EDK=db.extract(krige_ED,'Kriging.pol.stdev') # standard deviation
Pred_EDK=db.locate(krige_ED,9,NA)
StDev_EDK=db.locate(krige_ED,8,NA)
write.table(grid,file=paste(outdir2,"EDK_grid_drift500mNSLLURmin10DrivingDays_lag25_neigh1_500",city,"_",pol,"_",estim_YYYY,"_",kriging_data,'.csv',sep=''),row.names=F,col.names=T,sep=';')
...
Pred_EDK2=matrix(grid2$Pred_EDK,nrow=length(sort(unique(coordinates(grid2)[,1]))))
StDev_EDK2=matrix(grid2$StDev_EDK,nrow=length(sort(unique(coordinates(grid2)[,1]))))
min(StDev_EDK2)
min(Pred_EDK2)
max(StDev_EDK2)
max(Pred_EDK2)

Drift_matrix=matrix(grid2$Drift,nrow=length(sort(unique(coordinates(grid2)[,1]))))
...
ras_Pred=raster(list(x=sort(unique(coordinates(grid2)[,1])),y=sort(unique(coordinates(grid2)[,2])),z=Pred_EDK2))
...
ras_StDev=raster(list(x=sort(unique(coordinates(grid2)[,1])),y=sort(unique(coordinates(grid2)[,2])),z=StDev_EDK2))
...
data2.db=db.locate(data2.db,"pol","z")
data2.db=xvalid(data2.db,model=vario_res_drift_model,neigh.create(ndim=2,type=0),uc=c('1','f1'),radix='KDE_drift') # Perform cross-validation
data2.db=db.locate(data2.db,"KDE_drift.pol.stderr",NA)
data2$estim_KDE=data2$pol+db.extract(data2.db,'KDE_drift.pol.esterr')
save(data2,file=paste0(outdir2,"EDK_data_gt7res_lag25_",city,"_",pol,"_",estim_YYYY,"_",kriging_data,".Rda"))

# Do plot
png(filename=paste0(outdir,"CV_drift500mNSLLURmin10DrivingDays_lag25",city,"_",pol,"_",estim_YYYY,"_",kriging_data,"_VME.png"), width=1200, height=800, type="cairo",bg = "white")
