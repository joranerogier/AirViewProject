...
reg_VC<-function(formula,df) {
  ...
  for (i in 1:nrow(df)) {  
    ...
    #plot(reg$resid~df$pol[-i][order(df$pol[-i])],
    #     main="pol x Residuals\nfor Simple Regression",
    #     xlab="Pol", ylab="Residuals")
    #abline(h=0,lty=2)
    ...
    #print('---')
    #print(df$pol[i])
    #print(predict(reg,newdata=df[i,]))
    #print(df$Drift[i])
    #print(restmp)
    #print(df$pol[i] - df$Drift[i])
  }
  print(res)
  return(res)
}
...
vario_res_drift=vario.calc(data2.db, lag=100, nlag=40) # experimental variogram on residuals
