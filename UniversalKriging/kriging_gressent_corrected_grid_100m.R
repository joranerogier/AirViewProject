...
# Directory paths
indir_drift <- "C:/Users/Jorane Rogier/Documents/Master/DSthesis/Data/GridData/fishnet_AMS_100m_correctedNSL_UTR_and_AMS_LUR_predictions/"
...
file_drift <- "fishnet_AMS_100m_correctedNSL_UTR_and_AMS_LUR_predictions.shp"                                                            # Drift file
file_drift_points <- "fishnet_AMS_100m_correctedNSL_UTR_and_AMS_LUR_prediction_points1.csv"
...
(names(drift)[names(drift) == "mean_pred"] <- "pol")
drift_sp <- as(drift, Class="Spatial")
(names(drift_sp)[names(drift_sp) == "mean_pred"] <- "pol")
...
(names(drift_points)[names(drift_points) == "mean_pred"] <- "pol")
...
to.remove <- c("OID_", "NO2_Mixed", "DriveDays", "ROAD_FID","NO2_LUR","NO2_SE","NO2_Diff","NO2_SEp","Shape_Leng","ORIG_FID"  )
...
data_ms$round_POINT_X <- round(data_ms$POINT_X)
data_ms$round_POINT_Y <- round(data_ms$POINT_Y)
...
data_ms <- aggregate(.~round_POINT_X+round_POINT_Y,data=data_ms,mean,na.action=na.pass) # mean over position to avoid duplicate data on a similar location ==> error for kriging
...
zl_drift <- c(min(drift_points$pol),max(drift_points$pol))
...
reg=lm(data_RDnew$pol~data_RDnew$Drift,)
...
vario_res_drift=vario.calc(data2.db, lag=25) # experimental variogram on residuals
...
unique.neigh=neigh.create(ndim=2,type=2, nmini=2, nmaxi = 50, radius=c(500,500))# TODO: check this -> orginally this:neigh.create(type=0,ndim=2)
...
a### Prediction and mapping error
