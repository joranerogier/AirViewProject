...
vme2df <- function(obs_data, U) {
  ...
  for (l in 1:length(unique(obs_data$id_cell))) {
    ...
    pol_conc <- obs_data[which(obs_data$id_cell == cell), 3]$pol
    N <- length(pol_conc)
...
model="NSL-LUR-drift100m-airview-3_5_dd_lt25no2"                    # Dispersion model
...
file_airview <- "AMS_DataMixed_NO2_5Jul_MidPoints_ConcLT60_dd_3_5.csv"#
...
b <- 59.0
d <- 45.0
airview_ams$new_conc <- c + ((d-c)/(b-a)) * (airview_ams$NO2_Data - a)

(names(airview_ams)[names(airview_ams) == "new_conc"] <- "pol")
