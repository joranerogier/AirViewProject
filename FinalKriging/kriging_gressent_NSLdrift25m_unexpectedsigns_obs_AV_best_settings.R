...
# Maybe remove all values > 60 -> only +- 100-200 samples
...
drift_data = 'NSL_unexpectedSigns'
drift_cell_size = 25
drift_origin_city = 'AMS'
obs_data = 'AV'
obs_type = '100m_weighted_avg_winsorized_GTE3dd'
ums = 025
file_tag = paste(drift_data, '_', drift_cell_size, 'm_' , drift_origin_city, '_', obs_data, '_', obs_type,'_ums',ums, sep='')


# Directory paths
indir_drift <- "C:/Users/Jorane Rogier/Documents/Master/DSthesis/Data/GridData/fishnet_AMS_25m_NSL_LUR_predictions/"
indir_drift_points <- "C:/Users/Jorane Rogier/Documents/Master/DSthesis/Data/Predictions/" 
indir_airview <- "C:/Users/Jorane Rogier/Documents/Master/DSthesis/Data/GoogleAir/Amsterdam/"#intersection/
file_drift <- "fishnet_AMS_25m_NSL_LUR_predictions.shp"                                                            # Drift file
file_drift_points <- "fishnet_AMS_25m_NSL_LUR_predictions.csv"
...
outdir2 <- "C:/Users/Jorane Rogier/Documents/Master/DSthesis/Data/FinalPredictions/"# Directory for output predictions
outdir3 <- "C:/Users/Jorane Rogier/Documents/Master/DSthesis/Data/CV/"
...
res= 25                                         # Grid Resolution in meters
...
data1 <- rep(0,length(grid1$Var1)) # first attribute pseudo-data
grid1$data1 <- data1
...
drift_points <- read_csv2(file=paste0(indir_drift_points, file_drift_points))
...
#to.remove <- c( "IOD","Join_Count", "TARGET_FID", "ROAD_FID","NO2_LUR","NO2_Mixed",  "DriveDays",  "Shape_Leng",
#                "ORIG_FID","X","Y","CONC_NO2","res")
to.remove <- c("OID_", "NO2_Data", "NO2_Mixed", "DriveDays", "ROAD_FID","NO2_LUR","NO2_SE","NO2_Diff","NO2_SEp","Shape_Leng","ORIG_FID"  )
airview_ams <- airview_ams[,!names(airview_ams) %in% to.remove]

data_sub <- airview_ams[airview_ams$pol != 0,]

# decomment below if:winsorize data to remove outliers
data_sub$pol <- Winsorize(data_sub$pol)
# decomment below if: GTE 3 dd
data_sub <- data_sub[data_sub$sum_drivedays >= 3,]
...
par(mar=c(7,9,2,10)) # margin bot left top right (need space for the lgd
...
lag=100
nlag=50
vario_res_drift=vario.calc(data2.db, lag=lag, nlag=nlag) # experimental variogram on residuals
...
par(mar=c(7,9,2,9)) # margin bot left top right (need space for the lgd
plot(vario_res_drift, npairdw=TRUE,  npairpt=TRUE,main = "b)",ylim=c(0,150),lwd=3,cex.main=2,xlab="Distance h (m)",ylab="Î³(h)",cex.lab=1.8, cex.axis=1.8)
...
#unique.neigh=neigh.create(ndim=2,type=2, nmini=5, nmaxi = 50, radius=c(500,500))# TODO: check this -> orginally this:neigh.create(type=0,ndim=2)
...
write.table(grid,file=paste(outdir2, "EDK", file_tag,"_lag", lag, "nlag", nlag ,"uniqueneigh",'.csv',sep=''),row.names=F,col.names=T,sep=';')
...
# 
# c_pred <- which(Pred_EDK2>150, arr.ind=TRUE)
...
zl_fusion <- c(min(Pred_EDK2),max(Pred_EDK2))
...
zl_error <- c(min(StDev_EDK2)-0.1,max(StDev_EDK2)+0.1)
...
save(data2,file=paste0(outdir2,"CV_",file_tag.Rda))

# Do plot
png(filename=paste0(outdir,"CV_",city,"_",pol,"_",estim_YYYY,"_",kriging_data,"_VME.png"), width=1200, height=800, type="cairo",bg = "white")
